...
class HardWork {
  ...
  do() {
    let i = 0; // Counter to keep track of current task index
    let intervalId; // ID of the interval

    const executeNextTask = () => {
        if (i < this._tasks.length) {
            // Execute the next task if available
            this._tasks[i]();
            i++;
        }
        else {
            clearInterval(intervalId); // Stop the interval when all tasks are done
        }
    };
    ...
    // Stop executing tasks when the button is clicked
